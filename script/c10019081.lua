--Team Magma Belt (Team Magma vs Team Aqua 81/95)
local scard,sid=aux.GetID()
function scard.initial_effect(c)
	aux.AddSetcode(c,SETNAME_TEAM_MAGMA)
	--pokemon tool
	aux.EnablePokemonToolAttribute(c,aux.FilterBoolFunction(Card.IsSetCard,SETNAME_TEAM_MAGMA))
	--search (evolve)
...
function scard.con1(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsActive() and c:IsCanEvolve(e,tp,false,true) and c:IsSetCard(SETNAME_TEAM_MAGMA)
end
...
--[[
	...
	Q. Say I attach a Team Magma Belt to a benched TM's Aron. During my next turn I evolve it naturally into TM's Lairon,
	and then the following turn into TM's Aggron - all while on the bench. When I eventually move TM's Aggron into the
	active slot, what happens with the Team Magma Belt?
	A. Unfortunately, since you cannot evolve any higher than Aggron, the Team Magma Belt would remain attached until the
	Pokémon leaves play or otherwise devolves to the point that it could evolve again (i.e. Devolution Spray, Omastar's
	"Pull Down" attack, etc.). You would not be able to search, therefore you would not discard the Team Magma Belt.
	(Apr 22, 2004 PUI Rules Team)
