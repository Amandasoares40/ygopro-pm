...
--[[
	Rulings
		...
		A. Yes. Wally's Training can break the rules for normal evolution in that case. It also can allow a player to
		evolve a Pokémon the first turn it is in play, except for the very first turn of the game (for each player). In
		order for Wally's Training to break the "first turn of the game" evolution, the card would have to specifically
		say so. (Sep 26, 2003 PUI Rules Team)

		Q. Can I use Wally's Training on a Pokémon I just evolved that turn with Rare Candy?
		A. Yes. Trainer cards that allow an evolution to take place, breaks the standard evolution rule that states you
		can only evolve once in a turn. (Sep 26, 2003 PUI Rules Team)
		...
		Q. If you use Wally's Training to evolve a Pokémon, and the evo card has a "Coming into Play" Pokémon Power (such
		as Dark Crobat), does that coming into play power get activated?
		A. No, because those powers only work "when you play it from your hand". Wally's Training has you search your deck
		for the card. (Oct 23, 2003 PUI Rules Team)

		Q. Can I use Wally's Training to evolve a Baby Pokémon into its next stage? What about the newer forms of babies
		that are considered "Basic Pokémon" but have the "Baby Evolution" Poké-POWER?
		A. Wally's Training states if you have a card that "Evolves From" your Pokémon There are currently no cards with
		text saying "evolves from" its baby form, so Wally's Training will not work on any form of baby Pokémon
		(Oct 23, 2003 PUI Rules Team)

		Q. I evolved Torchic into Combusken using Wally's Training. Can I then do my "regular" evolution for Combusken
		into Blaziken?
		...
		text on it that says, "You may play a Pokémon card that evolves from Mysterious Fossil on top of Buried Fossil".
		So Wally's Training will work in both of these situations. (Jan 29, 2004 PUI Rules Team)

		Q. What does "This counts as evolving that Pokémon" mean, in game terms?
		A. It counts towards the one evolution per turn for that Pokémon (with exceptions for Trainers and Pokémon Powers
		that break that rule) and removes Special Conditions on that Pokémon. (Sep 26, 2003 PUI Rules Team)
